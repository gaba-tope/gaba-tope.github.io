...
events_data <- events_data |> 
  ...
  filter(year >= 1900)
```
 3. For enhanced readability in the plot's tooltip, I created a `tooltip` column. Using the [`base::strwrap()`](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/strwrap){:target="_blank"} function, I formatted the event descriptions with line breaks at word boundaries (determined by the specified `width` parameter). The lines were then combined using the `paste()` function, incorporating `<br>` HTML tags for line breaks. For those using tidyverse, [`stringr::str_wrap()`](https://stringr.tidyverse.org/reference/str_wrap.html){:target="_blank"} can be a good alternative for `base::strwrap()`.<br>
```r
events_data <- events_data |> 
  rowwise() |> 
...
<iframe src="/files/interactive_page/event_plotly_post.html"
 height="543px" width="768px" style="border:none;"></iframe>
```