...
<iframe src="/files/interactive_page/event_plotly.html" height="543px" width="768px" style="border:none;"></iframe>

<iframe src="/files/interactive_page/event_plotly_test.html" height="543px" width="768px" style="border:none;"></iframe>
...
events_data <- events_data |> 
  ...
  filter(year >= 1900)
```
 3. For enhanced readability in the plot's tooltip, I created a `tooltip` column. Using the `base::strwrap()` function, I formatted the event descriptions with line breaks at word boundaries (determined by the specified `width` parameter). The lines were then combined using the `paste()` function, incorporating `<br>` HTML tags for line breaks.<br>
```r
events_data <- events_data |> 
  rowwise() |> 
